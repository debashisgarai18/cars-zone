...
export async function getCarFilters() {
  try {
    ...
    return {
      success: true,
      data: {
        makes: makes.map((val) => val.make),
        bodyTypes: bodyTypes.map((val) => val.bodyType),
        fuelTypes: fuelTypes.map((val) => val.fuelType),
        transmissions: transmissions.map((val) => val.transmission),
        priceRange: {
...
export async function getCarById(carId) {
  try {
    ...
    throw new Error(`Error fetching car details by id : ${error.message}`);
  }
}
